...
def main(args, override_args=None):
    ...
    elif "datastore" not in global_vars():
        ...
        for subset in args.valid_subset.split(","):
            ...
            for i, sample in enumerate(progress):
                ...
                if knn_type in ["vanilla_knn_mt", "adaptive_knn_mt", "greedy_merge_knn_mt", "kernel_smoothed_knn_mt", "plac_knn_mt", "robust_knn_mt"]:
                    ...
                    if C > 1:
                        padded_target = torch.cat([target, target.new_full((B, C-1), pad_idx)], dim=-1)

                        valid_chunks = padded_target.unfold(dimension=1, size=C, step=1)[mask]
                        datastore["vals"].add(valid_chunks)
                    else:
                        non_pad_tokens, _ = filter_pad_tokens(target)
                        datastore["vals"].add(non_pad_tokens.unsqueeze(-1))
